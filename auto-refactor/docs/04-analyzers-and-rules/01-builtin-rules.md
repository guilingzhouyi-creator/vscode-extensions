...
| `CPX-NEST-001` | `warning` | 控制流嵌套深度超出上下文弹性预算（通用业务代码 > 3 层，状态机/解析器 > 5 层）。 | 利用卫语句（提前返回）扁平化控制流，或将复杂分支独立为子状态处理函数。 |
| `CPX-NEST-002` | `warning` | 在深层嵌套（>= 4 层）且距函数头长距离（> 40 行）处执行控制流逃逸（return/break/throw）。 | 局部卫语句提前校验，或将深层长跨度闭环提取为纯函数子算子以缩短认知跳跃距离。 |
| `CPX-STM-001` | `info` | 状态机多层分支内存在过长单分支逻辑（> 30 行），降低了状态转移分派骨架的清晰度。 | 将状态机单分支过长逻辑提取为独立动作处理器，保留纯粹的状态转移分派骨架。 |
| `CPX-HOP-001` | `warning` | 机械式拆分投机：通过制造大量单行薄转发包装函数人为压低复杂度。 | 消除无意义的透传转发包装，聚焦于语义重用与领域内聚。 |
