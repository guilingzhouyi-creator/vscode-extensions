#!/usr/bin/env node
/**
 * Module: Verification Harness — Empirical Quantitative Benchmark Suite
 * File Path: scripts/bench-quant.js
 * Architecture Role: Measurement-only harness over the built dist API; it reports
 *   hardware-aligned timings instead of gating any pass/fail threshold.
 * Dependencies & Triggers: ../dist/api and ../dist/core/utf8 bundles; triggered by
 *   `npm run bench-quant` (node scripts/bench-quant.js); --expose-gc strengthens
 *   the heap-delta measurement by enabling the explicit global.gc() call.
 * Responsibilities: Generate paired TypeScript-like sources at configurable edit
 *   spacing; warm up and take the median of repeated runs via process.hrtime;
 *   cover multi-scale Myers vs FastDiff, disjoint worst cases, single-pass line
 *   indexing, SWAR ASCII throughput (8 bytes/cycle), 64-bit Bit-Parallel Myers,
 *   50k-line scaling, 500-run memory throughput, and 1000-node dependency-graph
 *   queries.
 * Exit Semantics & Design Rationale: Exits 0 after printing all nine benchmark
 *   tables; exits 1 with 'Benchmark error:' if any measurement throws. Results stay
 *   advisory numbers so timing noise remains diagnosable rather than blocking CI.
 */
...
function generateSyntheticSource(lineCount, editSpacing = 50) {
  ...
  for (let i = 1; i <= lineCount; i++) {
    if (i % editSpacing === 0) {
      ...
      newLines.push(
        `  function calculateMetric_${i}(val: number): number { return val * 42 + 100; }`,
      );
    } else {
...
async function main() {
  ...
  for (const count of scales) {
    ...
    const a = oldIdx.starts.map((_, i) =>
      oldContent.slice(
        oldIdx.starts[i],
        oldIdx.starts[i + 1] ? oldIdx.starts[i + 1] - 1 : oldContent.length,
      ),
    );
    const b = newIdx.starts.map((_, i) =>
      newContent.slice(
        newIdx.starts[i],
        newIdx.starts[i + 1] ? newIdx.starts[i + 1] - 1 : newContent.length,
      ),
    );
  ...
  // --- BENCHMARK 2: Myers Flat Int32Array Trace (Disjoint Worst-Case) ---
  console.log('\n2. Myers Disjoint Worst-Case (Zero Common Lines):');
  console.log('----------------------------------------------------------------');
  ...
  console.log(
    `- 1000 lines completely disjoint (Baseline 40.17ms): ${tDisjoint1000.toFixed(3)} ms`,
  );

  // --- BENCHMARK 3: Single-Pass computeLineStartsAndHashes ---
  console.log('\n3. computeLineStartsAndHashes Single-Pass vs computeEditRanges:');
  console.log('----------------------------------------------------------------');
  const src5000 = generateSyntheticSource(5000);
  const tSinglePass = timeFn(() => computeLineStartsAndHashes(src5000.oldContent), 5, 10);
  const tRangesWithOps = timeFn(
    () => computeEditRangesWithOps(src5000.oldContent, src5000.newContent),
    5,
    10,
  );
  ...
  // --- BENCHMARK 4: Dispersed 10-Edit vs 1-Edit on 5000 Lines ---
  console.log('\n4. Dispersed 10-Edit vs 1-Edit on 5000 Lines:');
  console.log('----------------------------------------------------------------');
  ...
  const t10Edit = timeFn(
    () => computeEditRanges(src10Edit.oldContent, src10Edit.newContent),
    5,
    10,
  );
  ...
  console.log(
    `- 64-bit SWAR isPureAscii     : ${tSwarAscii.toFixed(4)} ms (${(asciiBuf.length / 1024 / 1024 / (tSwarAscii / 1000)).toFixed(1)} MB/s throughput)`,
  );
  ...
  const text64 = new Uint32Array(
    Array.from({ length: 64 }, (_, i) => (i % 5 === 0 ? i * 9999 : i * 1337)),
  );
  ...
  console.log(
    `- 64-line Pattern Bit-Parallel Myers : ${tBpm.toFixed(4)} ms (${(tBpm * 1000).toFixed(1)} μs)`,
  );
  ...
  console.log(
    `- 50,000 Lines File Size             : ${(src50k.oldContent.length / 1024 / 1024).toFixed(2)} MB`,
  );
  ...
  for (let i = 0; i < 1000; i++) {
    ...
    const imports = [`./module_${Math.max(0, i - 1)}.ts`, `./module_${Math.max(0, i - 2)}.ts`];
