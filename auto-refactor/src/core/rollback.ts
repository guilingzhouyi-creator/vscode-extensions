/**
 * Module: Core Engine — Rollback Primitives & Merge Gate
 * File Path: src/core/rollback.ts
 * Architecture Role: Pure hunk/card rollback functions plus PraxisRollbackEngine, the
 *     IPraxisRollbackGatekeeper implementation used as the Praxis merge-gate hook.
 * Dependencies & Triggers: ./praxis/contracts (ReviewDiffHunk, IPraxisRollbackGatekeeper) and
 *     ./editDiff (computeLineStarts, linesOf); exported through api.ts and exercised by
 *     scripts/validate-praxis-foundation.js.
 * Responsibilities: Validate a hunk's new span, splice its old lines back and emit the
 *     reversed unified patch; cascade-revert every hunk attributed to a cardId in descending
 *     line order while collecting checkpoint ids; index files/hunks/cards; reject merges
 *     whose hunks demand L3A escalation or major rework.
 * Exit Semantics & Design Rationale: revertDiffHunk never throws and never mutates the input
 *     content — failures return { success: false, error }; revertTaskCard stops a file at its
 *     first failed hunk and reports only fully reverted files as affected; checkMergeGate
 *     returns a reason plus violation list instead of throwing.
 *
 ...
 * `success` gates whether `updatedContent` is safe to use; `patch` always carries the unified
 * reversal text when one could be built, and `error` explains a failed revert.
 */
...
/**
 * Outcome of cascading rollback for every hunk attributed to one task card.
 * `affectedFiles` and `updatedFiles` list only files whose hunks were fully reverted; `success`
 * reports that the cascade ran, not that every matching file was restored.
 */
 ...
 * The input content is never mutated, and any failure is captured instead of thrown.
 *
 * @param currentContent - Text the hunk was applied to; its newline style is normalized to `\n`.
 * @param hunk - Diff hunk whose `newSpan` locates the region to restore.
 * @returns The reverted text and reversal patch on success, or an error description on failure.
 */
export function revertDiffHunk(currentContent: string, hunk: ReviewDiffHunk): RollbackResult {
    try {
 ...
 * descending start-line order so earlier offsets stay valid, and a file stops at its first
 * failed hunk so a partial revert is never reported as an affected file.
 *
 * @param fileContents - Current text per file path; paths absent from the map are skipped.
 * @param targetCardId - Card whose attributed hunks should be reverted.
 * @param hunksByFile - Diff hunks per file; entries without a matching hunk are ignored.
 * @returns Affected paths, checkpoint ids from reverted hunks, and the fully reverted texts.
 */
    ...
    checkMergeGate(
        ...
        for (const hunk of diffPayload) {
            if (hunk.reviewVerdict?.shouldEscalateToL3A) {
                violations.push(
                    `Hunk ${hunk.hunkId} marked as major rework requiring L3A escalation`,
                );
            }
