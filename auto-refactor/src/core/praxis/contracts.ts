/**
 * Module: Core Engine — Praxis Extension Contracts & Service Provider Interface (SPI)
 * File Path: src/core/praxis/contracts.ts
 * Architecture Role: Compile-time single source of truth for Praxis card, diff,
 *   verdict, enrichment, and plugin-hook shapes consumed by the default implementations
 *   and the public barrel; declares no runtime values, so it adds zero streaming overhead.
 * Dependencies & Triggers: No imports. TypeScript loads it when resolving `./defaults`,
 *   `./index`, or external SPI implementations; every declaration is an interface or type
 *   alias, so loading triggers no side effects.
 * Responsibilities: Define generic `PraxisCardContext`/`PraxisPluginHooks` payloads; model
 *   attributed diff lines and review hunks with AST/LSP context and RC metrics; encode
 *   `passed`/`minor_fix_needed`/`major_rework_needed` verdicts plus L3A escalation flags;
 *   and specify the five SPI ports for attribution, enrichment, threshold policy,
 *   human-facing ring-buffer storage, and rollback/merge gating.
 * Exit Semantics & Design Rationale: Type-only module evaluation cannot throw and leaves
 *   no runtime state; keeping contracts free of implementation lets both built-in
 *   defaults and custom plugins share one contract language without bundling either side.
 *
