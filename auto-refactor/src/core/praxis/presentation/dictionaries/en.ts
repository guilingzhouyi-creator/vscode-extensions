...
export const EN_RULES: Record<string, PraxisRuleI18nEntry> = {
    ...
    'ADV-PRF-001': {
        ...
        remediation: 'Hoist loop-invariant expressions outside the loop and cache in a local variable.',
    ...
    'ADV-PRF-002': {
        ...
        summary: 'High-frequency temporary object allocation inside tight loop causing GC pressure.',
        remediation: 'Pre-allocate reusable object or pool outside the loop and invoke reset_state().',
        rationale: 'Short-lived objects trigger frequent garbage collection pauses, hurting latency.',
    },
    'ARC-COH-001': {
        name: 'File Line Count Exceeds Limit',
        summary: 'File physical line count exceeds architecture threshold (standard limit: 400 lines).',
        remediation: 'Split responsibilities according to Single Responsibility Principle into cohesive submodules.',
    ...
    'ARC-COH-002': {
        ...
        summary: 'Total number of top-level functions and methods exceeds budget (standard limit: 20).',
        remediation: 'Group related helper functions and extract them to dedicated utility or domain classes.',
        rationale: 'High function counts dilute architectural clarity and risk creating God Objects.',
    },
    'CMP-LOC-001': {
        ...
        remediation: 'Use guard clauses with early returns or table-driven dispatch to flatten branching.',
        rationale: 'High branching complexity exponentially expands test matrix and obscures edge bugs.',
    },
    'CMP-LOC-002': {
        name: 'Excessive Block Nesting Depth',
        summary: 'Control flow nesting level exceeds maximum allowable budget (standard limit: 4 levels).',
        remediation: 'Extract deep nested blocks into private functions and adopt early return style.',
    ...
    'TYP-ANY-001': {
        ...
        remediation: 'Replace any with specific interfaces, generics, union types, or unknown + Type Guards.',
        rationale: 'Escaped any disables TypeScript compiler safety checks and defers errors to runtime.',
    },
    'RES-LAK-001': {
        name: 'Resource Not Guaranteed Released',
        summary: 'Open streams, handles, timers, or locks not guaranteed freed in finally block.',
        remediation: 'Wrap resource lifecycle in try...finally and call dispose/close, or use Disposable pattern.',
    ...
    'ASY-AWT-001': {
        ...
        remediation: 'Add await at call site or handle rejection with .then().catch() promise chains.',
    ...
    'MOD-EXP-001': {
        ...
        rationale: 'Wildcard re-exports increase name collisions and enlarge final distribution bundles.',
    },
};
