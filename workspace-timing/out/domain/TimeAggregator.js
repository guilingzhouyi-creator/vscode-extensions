...
class TimeAggregator {
     ...
     */
    static addToHourly(hourMap, startMs, endMs) {
        let cursor = startMs;
        let first = true;
        while (cursor < endMs) {
            const d = new Date(cursor);
            const nextHourStart = new Date(d.getFullYear(), d.getMonth(), d.getDate(), d.getHours() + 1).getTime();
    ...
    /** 周报文字摘要 */
    static weeklySummary(sessions, currentSessionStartMs = 0) {
        const now = Date.now();
        ...
        const dayMap = new Map();
        const accumulate = (startMs, endMs) => {
            if (startMs >= weekEndMs || endMs <= weekStartMs)
                return;
            TimeAggregator.eachDaySegment(Math.max(startMs, weekStartMs), Math.min(endMs, weekEndMs), (date, segStart, segEnd) => {
        ...
        for (const s of sessions) {
            if (s.startMs >= weekEndMs || s.endMs <= weekStartMs)
                continue;
            sessionCount++;
