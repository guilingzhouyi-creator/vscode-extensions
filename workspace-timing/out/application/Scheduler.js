...
class Scheduler {
    ...
    updateIntervals(patch) {
        ...
        if (!journalChanged && !fullSaveChanged)
            return;
        ...
        (0, Logger_1.log)(Logger_1.LogLevel.Debug, `Scheduler: intervals updated (journal=${this.options.journalFlushIntervalMs}ms, fullSave=${this.options.fullSaveIntervalMs}ms)`);
    }
    /** 启动所有周期任务 */
    start() {
        if (this._running)
