...
export class CsvExporter implements IDataExporter {
    ...
    exportDashboard(data: DashboardData, workspaceName: string): string {
        ...
        for (const d of data.dailyStats) {
            const total = TimeAggregator.formatDuration(d.totalMs);
            const active = d.activeMs !== undefined ? TimeAggregator.formatDuration(d.activeMs) : '-';
            const idle = d.idleMs !== undefined ? TimeAggregator.formatDuration(d.idleMs) : '-';
