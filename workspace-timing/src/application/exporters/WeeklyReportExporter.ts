...
export class WeeklyReportExporter {
    ...
    generate(input: WeeklyReportInput): string {
        ...
        const fmt = TimeAggregator.formatDuration;
        const todayStr = localDateStr(input.generatedAt);
        ...
        if (hasGoal) {
            for (const d of daily) {
                if (d.dateStr > todayStr) continue; // 未来天不计
                if (d.totalMs >= input.dailyGoalMs && d.totalMs > 0) goalDays++;
            }
        }
        ...
        lines.push(`生成时间：${localDateStr(input.generatedAt)} ${pad(input.generatedAt.getHours())}:${pad(input.generatedAt.getMinutes())}`);
        lines.push('');
        lines.push('## 概览');
    ...
    private effLine(eff?: number): string {
        if (eff === undefined) return '未启用效率统计';
