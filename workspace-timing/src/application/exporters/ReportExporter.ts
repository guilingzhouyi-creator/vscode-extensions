...
export class ReportExporter {
    ...
    static buildDailyReport(detail: DailyDetail): string {
        ...
        lines.push('');
        lines.push(`- **当日时长**：${TimeAggregator.formatDuration(detail.totalMs)}`);
    ...
    static buildWeeklyReport(
        ...
        lines.push('## 本周摘要');
        lines.push('');
        lines.push(`- **本周总时长**：${TimeAggregator.formatDuration(summary.totalMs)}`);
        lines.push(`- **日均时长**：${TimeAggregator.formatDuration(summary.avgDailyMs)}`);
        lines.push(`- **活跃天数**：${summary.activeDays} 天`);
        lines.push(`- **最活跃日期**：${summary.peakDate}（${TimeAggregator.formatDuration(summary.peakDateMs)}）`);
        ...
        if (dailyStats.length > 0) {
            ...
            for (const d of dailyStats) {
                lines.push(`| ${d.label} | 周${d.weekday} | ${TimeAggregator.formatDuration(d.totalMs)} |`);
            }
            lines.push('');
        }
