...
## 停机第二阶段：全域持久化落盘与存盘超时判定
static func _execute_shutdown_save_stage(request: GameShutdownDTO.Request, providers: Dictionary, start_time: int, fsm: Variant, resp: GameShutdownDTO.Response, bus: Variant, save_timeout: float) -> void:
...
## 停机第三/四阶段：会话注销与反装配清理
static func _execute_shutdown_teardown_stage(request: GameShutdownDTO.Request, start_time: int, fsm: Variant, resp: GameShutdownDTO.Response, bus: Variant, shutdown_timeout: float, force_timeout: float) -> void:
...
static func execute_graceful_shutdown(request: GameShutdownDTO.Request, providers: Dictionary = {}) -> GameShutdownDTO.Response:
	...
	_execute_shutdown_save_stage(request, providers, start_time, fsm, resp, bus, save_timeout)
	_execute_shutdown_teardown_stage(request, start_time, fsm, resp, bus, shutdown_timeout, force_timeout)
