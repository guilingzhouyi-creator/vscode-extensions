#!/usr/bin/env bash
# ==============================================================================
# 模块归属: CI/CD 自动化流水线 (Release · 发布自动化框架)
# 文件路径: scripts/sh/release.sh
# 架构定位: 自动化发布编排器 (Linux Bash)
# 依赖与触发: 触发方: CI 标签触发 | 上游: pr-gate.sh / package.sh | 下游: Release 产物分发 | 运行时: Bash 4+
# 职责说明: 自动化响应合流与版本标签事件，驱动产物编译、幂等去重与多平台分发
# 退出语义与设计依据: 退出码: 0=发布成功, 1=流水线失败 | 设计依据: 自动化持续交付工程契约
# ------------------------------------------------------------------------------
# 用法示例:
